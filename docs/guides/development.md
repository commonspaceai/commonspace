...
For repeatable latency measurements through the real Laya worker and local router, run `pnpm benchmark:laya`. It builds the worker, warms four labeled routing paths once, then measures 30 sequential rounds of named and group greetings, explicit parallel recipients, and a named Project scope attempt. The JSON output includes model-manifest and machine details, worker readiness time, per-path acceptance counts, raw samples, and p50/p95/max latency. An abstained route still measures its Laya call and is visible in the acceptance count. Each timed call includes routing, worker IPC, tokenization, and ONNX inference; readiness is separate and may include a 613 MB download on a cold cache. Run on an otherwise idle machine and use `pnpm evaluate:classifier` for broader routing quality checks. Pass an optional cache directory after `benchmark:laya` to isolate model files.

Run `pnpm benchmark:routing-latency`. This provider-free service benchmark drives durable Channel sends through `CommonspaceHostService` with controlled classifier and Harness decisions and stub workers. It reports separate local-accepted, local-abstained, classifier-unavailable, classifier-error, correction-forced fallback, and parallel-fallback branches. The first sample uses a fresh Channel (or a saved correction); the subsequent 20 samples reuse that Channel. The primary clock starts at the durable pending-routing revision, since routing can begin before `send()` returns; response-return time is recorded separately. Each branch records persisted routing resolution, first and last worker start, raw samples, and p50/p95/max for the warm samples. Its controls require the expected owner, routing source, participant set, and classifier/fallback call counts; injected classifier and fallback delays must move only their matching path, while downstream worker delay must not inflate dispatch timing. Set `COMMONSPACE_ROUTING_BENCHMARK_REPETITIONS` to 1–30 to change warm sample count; small counts make p95 close to or equal to max. The results exclude real model inference and provider latency; use `benchmark:laya` and `evaluate:classifier` for Laya component evidence, and keep live Harness timing separate.
