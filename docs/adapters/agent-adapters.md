...
| OpenCode    | User MCP configuration names from XDG and `~/.opencode`, including JSONC, and global skill folders; native OAuth token status and sign-in through `opencode mcp auth` when supported; environment/project overrides are excluded |
...
For Codex, `codex mcp list --json` supplies `logged_in`, `not_logged_in`, `unsupported`, or `unknown` OAuth state. `not_logged_in` does not prove the server requires OAuth; `unsupported` does not rule out other credentials. Agent settings offer Authenticate or Reauthenticate for the first two states, and `codex mcp login <name>` runs only after the adapter confirms the name and current status.

For OpenCode, `opencode mcp auth list --pure` reports `authenticated`, `expired`, or `not authenticated` from native OAuth token metadata without connecting to MCP servers. Missing tokens do not prove the server requires OAuth. Refresh runs that command with a disposable home/config/cache/state and a minimal configuration containing only the configured remote server names and URLs; it preserves the native data directory so OpenCode can read its own token state. This prevents the CLI's startup writes from changing user configuration. The adapter keeps only validated names and statuses whose native URL matches the user configuration; an unrecognized listing leaves authentication unreported. It omits OAuth status when `OPENCODE_CONFIG`, `OPENCODE_CONFIG_DIR`, or `OPENCODE_CONFIG_CONTENT` overrides that configuration, or when a server URL uses `{env:...}` or `{file:...}` substitution. The last case is called out in the inventory notice. After a user clicks Authenticate or Reauthenticate, it checks the effective native URL and status before `opencode mcp auth <name> --pure`, then confirms the command's success and refreshed status. The command's renewal prompt is answered through piped input. Other credential methods and connection health are not inferred. Claude Code and Gemini CLI's MCP list commands run health checks or start servers; Hermes offers reauthentication but its MCP list command does not report sign-in state. Those adapters leave authentication unreported.
